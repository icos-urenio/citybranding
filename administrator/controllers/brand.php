 ...
 */
class CitybrandingControllerBrand extends JControllerForm
{
    ...
    {
/*        //A: inform log table about the new brand
        if($validData['id'] == 0){
            
            $log = JTable::getInstance('Log', 'CitybrandingTable', array());

            $catTitle = CitybrandingFrontendHelper::getCategoryNameByCategoryId($validData['areaid']);

            $data2['id'] = 0;
            $data2['state'] = 1;
            $data2['action'] = JText::_('COM_CITYBRANDING_LOGS_ACTION_INITIAL_COMMIT');
            $data2['brandid'] = $model->getItem()->get('id');
            ...
            try
            {
                ...
                $results = $dispatcher->trigger( 'onAfterNewBrandAdded', array( $model, $validData ) );
    ...
    /*
    public function printBrand($pk = null)
    {
        // Get the input
        $input = JFactory::getApplication()->input;
        $brandid = $input->get('id', 0);
        $model = $this->getModel();
        $model->setState('printid', $brandid);

        $v = $this->getView('brand', 'print');              //view.print.php
        $v->setModel($model, true);                         //load brand model
        $v->setModel($this->getModel('Logs', 'CitybrandingModel'));  //load logs as well
        $v->display('print');                               //default_print   

        // Redirect to the list screen.
        //$this->setRedirect(JRoute::_('index.php?option=com_citybranding&view=brand&layout=edit&id='.$brandid, false));
    }
    */
