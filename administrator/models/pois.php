...
/*        if (JFactory::getApplication()->isAdmin())
    ...
    public function isOwnPoi($poiid, $userid) {
        $db = JFactory::getDbo();
        $query = $db->getQuery(true);
        $query->select('COUNT(*)');
        $query->from('`#__citybranding_pois` AS a');
        $query->where('a.id    = ' . $db->quote($db->escape($poiid)));
    ...
    public function updateVotes($poiid, $userid) {
        $db = JFactory::getDbo();        
        $query = $db->getQuery(true);
        $query  ->update($db->quoteName('#__citybranding_pois'))
                ->set($db->quoteName('votes') .'=' . $db->quoteName('votes') . ' + 1')
                ->where($db->quoteName('id') .'='. $poiid);
        $db->setQuery($query);
        $results = $db->execute();
        return $results;
    }

    public function getVotes($poiid) {
        $db = JFactory::getDbo();  
        $query = $db->getQuery(true);
        $query->select('a.votes');
        $query->from('`#__citybranding_pois` AS a');
        $query->where('a.id = '. $poiid);
        $db->setQuery($query);
        $results = $db->loadResult();
        return $results;
    }
