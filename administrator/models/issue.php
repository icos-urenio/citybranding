 ...
 * Citybranding model.
 */
class CitybrandingModelIssue extends JModelAdmin
{
	/**
	 * @var		string	The prefix to use with controller messages.
	 * @since	1.6
	 */
	protected $text_prefix = 'COM_CITYBRANDING';
	...
	public function getForm($data = array(), $loadData = true)
	{
		...
		// Get the form.
		$form = $this->loadForm('com_citybranding.issue', 'issue', array('control' => 'jform', 'load_data' => $loadData));
		if (empty($form)) {
			return false;
		}
		...
		if ($id != 0 && (!$user->authorise('core.edit.state', 'com_citybranding.issue.' . (int) $id))
			|| ($id == 0 && !$user->authorise('core.edit.state', 'com_citybranding'))
		)
		{
	...
	public function getItem($pk = null)
	{
		if ($item = parent::getItem($pk)) {
	        ...
	        $category = JCategories::getInstance('Citybranding')->get($item->catid);
	...
	protected function prepareTable($table)
	{
		...
		if (empty($table->id)) {
			...
			if (@$table->ordering === '') {
				$db = JFactory::getDbo();
				$db->setQuery('SELECT MAX(ordering) FROM #__citybranding_issues');
				$max = $db->loadResult();
