 ...
 * Citybranding model.
 */
class CitybrandingModelBrand extends JModelAdmin
{
	/**
	...
	public function getForm($data = array(), $loadData = true)
	{
		...
		// Get the form.
		$form = $this->loadForm('com_citybranding.brand', 'brand', array('control' => 'jform', 'load_data' => $loadData));
		if (empty($form)) {
			return false;
		}
		...
		if ($id != 0 && (!$user->authorise('core.edit.state', 'com_citybranding.brand.' . (int) $id))
...
//			}
	...
	protected function prepareTable($table)
	{
		...
		if (empty($table->id)) {
			...
			if (@$table->ordering === '') {
				$db = JFactory::getDbo();
				$db->setQuery('SELECT MAX(ordering) FROM #__citybranding_brands');
				$max = $db->loadResult();
