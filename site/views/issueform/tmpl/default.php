...
<?php if ($firstStep['ordering'] != 1 && isset($this->item->id) && $this->item->id > 0) :?>
	<div class="alert alert-danger"><?php echo JText::_('COM_CITYBRANDING_ISSUE_CANNOT_EDIT_ANYMORE'); ?></div>
	<?php return; ?>	
<?php endif; ?>
...
if($this->item->id) {
	$canState = JFactory::getUser()->authorise('core.edit.state','com_citybranding.issue');
} else {
	$canState = JFactory::getUser()->authorise('core.edit.state','com_citybranding.issue.'.$this->item->id);
}
...
<div class="issue-edit front-end-edit">
    <form id="form-issue" action="<?php echo JRoute::_('index.php?option=com_citybranding&task=issue.save'); ?>" method="post" class="form-validate form-horizontal" enctype="multipart/form-data">
    <div style="float:right;">
        <button type="submit" class="validate btn btn-primary"><i class="icon-ok"></i> <?php echo JText::_('JSUBMIT'); ?></button>
        <a class="btn" href="<?php echo JRoute::_('index.php?option=com_citybranding&task=issueform.cancel'); ?>" title="<?php echo JText::_('JCANCEL'); ?>"><?php echo JText::_('JCANCEL'); ?></a>
    </div>

    <?php if (!empty($this->item->id)): ?>
        <h1><i class="icon-pencil"></i> <?php echo JText::_('COM_CITYBRANDING_ISSUE_EDIT'); ?> #<?php echo $this->item->id; ?></h1>
    <?php else: ?>
        <h1><i class="icon-plus-sign"></i> <?php echo JText::_('COM_CITYBRANDING_ISSUE_NEW'); ?></h1>
    <?php endif; ?>
