...
<script type="text/javascript">
    ...
    function deleteItem() {
        if (confirm("<?php echo JText::_('COM_CITYBRANDING_DELETE_MESSAGE'); ?>")) {
            window.location.href = '<?php echo JRoute::_('index.php?option=com_citybranding&task=poi.remove&id=' . $this->item->id, false, 2); ?>'
        }
    }
...
<?php if ($this->item && ($this->item->state == 1 || $canEditOwn ) ) : ?>
    ...
    <div class="row">
	    ...
	    <div class="citybranding-poi-title">
		    ...
		    <?php if($canEdit && $this->item->checked_out == 0 && $canEditOnStatus): ?>
				<a class="btn btn-default btn-xs" href="<?php echo JRoute::_('index.php?option=com_citybranding&task=poi.edit&id='.$this->item->id); ?>"><?php echo JText::_("COM_CITYBRANDING_EDIT_ITEM"); ?></a>
			<?php endif; ?>
			<?php /*if(JFactory::getUser()->authorise('core.delete','com_citybranding.poi.'.$this->item->id)):?>
	    ...
	    <div class="col-lg-6 col-sm-12 col-xs-12 col-lg-pull-6">
			<div class="citybranding-info-wrapper">
		    	...
		    	<p>
		    		<strong><?php echo JText::_('COM_CITYBRANDING_POIS_CATID'); ?>: </strong> 
		    		<?php echo $this->item->catid_title; ?><br />

					<strong><?php echo JText::_('COM_CITYBRANDING_FORM_LBL_POI_CREATED'); ?>: </strong> 
					<?php echo CitybrandingFrontendHelper::getRelativeTime($this->item->created); ?><br />

		    		<?php if ($this->params->get('showuserdetailstimeline')) : ?>
			    		<strong><?php echo JText::_('COM_CITYBRANDING_FORM_LBL_POI_CREATED_BY'); ?>: </strong> 
			    		<?php echo $this->item->created_by_name; ?>
		    		<?php endif ?>
    		
		    		<?php if ($this->item->regnum != '') : ?>
			    		<strong><?php echo JText::_('COM_CITYBRANDING_FORM_LBL_POI_REGNUM'); ?>: </strong> 
			    		<?php echo $this->item->regnum; ?><br />
		    		<?php endif; ?>
	    			
	    			<?php if ($this->item->regdate != '0000-00-00 00:00:00') : ?>		    		
		    			<strong><?php echo JText::_('COM_CITYBRANDING_FORM_LBL_POI_REGDATE'); ?>: </strong> 
		    			<?php echo  date("d-m-Y", strtotime($this->item->regdate)); ?><br />
					<?php endif; ?>
		    		
		    	</p>
		    	<div class="citybranding-poi-subtitle"><?php echo JText::_('COM_CITYBRANDING_FORM_LBL_POI_DESCRIPTION'); ?></div>
				...
				<?php if(!empty($photos->files) && file_exists($photos->imagedir .'/'. $photos->id . '/thumbnail/' . (@$photos->files[0]->name))) : ?>
					<div class="citybranding-poi-subtitle"><?php echo JText::_('COM_CITYBRANDING_POI_PHOTOS'); ?></div>
					<div id='gallery'>
					<?php $count = 1; ?>
					<?php foreach ($photos->files as $photo) : ?>
				        <a href="<?php echo $photos->imagedir .'/'. $photos->id . '/' . ($photo->name) ;?>">
				        	<?php if($count == 1) : ?>
					        	<img src="<?php echo $photos->imagedir .'/'. $photos->id . '/medium/' . ($photo->name) ;?>" alt="<?php echo JText::_('COM_CITYBRANDING_POIS_PHOTO') . ' '. $count;?>" class="img-responsive" /><br />
				        	<?php else :?>
				        		<img src="<?php echo $photos->imagedir .'/'. $photos->id . '/thumbnail/' . ($photo->name) ;?>" alt="<?php echo JText::_('COM_CITYBRANDING_POIS_PHOTO') . ' '. $count;?>" class="img-responsive" />
				        	<?php endif; ?>
				        </a>
				        <?php $count++;?>
					<?php endforeach; ?>
    ...
    <div class="row">
		...
		<button id="timeline" class="btn btn-primary btn-lg" type="button" data-target="#cd-timeline" aria-expanded="false" aria-controls="cd-timeline">
		  <?php echo JText::_('COM_CITYBRANDING_POI_TIMELINE'); ?> <i class="icon-arrow-down"></i>
		</button>			
		</div>
				...
				<div class="cd-timeline-block">
					...
					<div class="cd-timeline-content">
						...
						<?php if($this->params->get('showuserdetailstimeline') && $first) : ?>
							<p><strong><?php echo JText::_('COM_CITYBRANDING_FORM_LBL_POI_CREATED_BY'); ?>: </strong> 
...
<?php /*
    <div class="item_fields2">
        <table class="table">
            <tr>
			<th><?php echo JText::_('COM_CITYBRANDING_FORM_LBL_POI_ID'); ?></th>
			<td><?php echo $this->item->id; ?></td>
			</tr>
			<tr>
			<th><?php echo JText::_('COM_CITYBRANDING_FORM_LBL_POI_TITLE'); ?></th>
			<td><?php echo $this->item->title; ?></td>
			</tr>
			<tr>
			<th><?php echo JText::_('COM_CITYBRANDING_FORM_LBL_POI_STEPID'); ?></th>
			<td><?php echo $this->item->stepid_title . ' -> '. $this->item->stepid;; ?></td>
			</tr>
			<tr>
			<th><?php echo JText::_('COM_CITYBRANDING_FORM_LBL_POI_CATID'); ?></th>
			<td><?php echo $this->item->catid_title; ?></td>
			</tr>
			<tr>
			<th><?php echo JText::_('COM_CITYBRANDING_FORM_LBL_POI_DESCRIPTION'); ?></th>
			<td><?php echo $this->item->description; ?></td>
			</tr>
			<tr>
			<th><?php echo JText::_('COM_CITYBRANDING_FORM_LBL_POI_ADDRESS'); ?></th>
			<td><?php echo $this->item->address; ?></td>
			</tr>
<!-- 			
			<tr>
			<th><?php echo JText::_('COM_CITYBRANDING_FORM_LBL_POI_LATITUDE'); ?></th>
			<td><?php echo $this->item->latitude; ?></td>
			</tr>
			<tr>
			<th><?php echo JText::_('COM_CITYBRANDING_FORM_LBL_POI_LONGITUDE'); ?></th>
			<td><?php echo $this->item->longitude; ?></td>
			</tr>
			<tr>
			<th><?php echo JText::_('COM_CITYBRANDING_FORM_LBL_POI_PHOTOS'); ?></th>
			<td><?php echo $this->item->photo; ?></td>
			</tr>
 -->
 			<tr>
			<th><?php echo JText::_('COM_CITYBRANDING_FORM_LBL_POI_STATE'); ?></th>
			<td>
			...
			<tr>
			<th><?php echo JText::_('COM_CITYBRANDING_FORM_LBL_POI_CREATED'); ?></th>
			<td><?php echo $this->item->created; ?></td>
			</tr>
			<tr>
			<th><?php echo JText::_('COM_CITYBRANDING_FORM_LBL_POI_UPDATED'); ?></th>
			<td><?php echo $this->item->updated; ?></td>
			</tr>
			<tr>
			<th><?php echo JText::_('COM_CITYBRANDING_FORM_LBL_POI_CREATED_BY'); ?></th>
			<td><?php echo $this->item->created_by_name; ?></td>
			</tr>
<!-- 			<tr>
			<th><?php echo JText::_('COM_CITYBRANDING_FORM_LBL_POI_LANGUAGE'); ?></th>
			<td><?php echo $this->item->language; ?></td>
			</tr> -->
			<tr>
			<th><?php echo JText::_('COM_CITYBRANDING_FORM_LBL_POI_HITS'); ?></th>
			<td><?php echo $this->item->hits; ?></td>
			</tr>
			<tr>
			<th><?php echo JText::_('COM_CITYBRANDING_FORM_LBL_POI_NOTE'); ?></th>
			<td><?php echo $this->item->note; ?></td>
			</tr>
			<tr>
			<th><?php echo JText::_('COM_CITYBRANDING_FORM_LBL_POI_VOTES'); ?></th>
			<td><?php echo $this->item->votes; ?></td>
			</tr>
<!-- 			<tr>
			<th><?php echo JText::_('COM_CITYBRANDING_FORM_LBL_POI_MODALITY'); ?></th>
