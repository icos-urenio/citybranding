 ...
 * Citybranding model.
 */
class CitybrandingModelPoiForm extends JModelForm
{
	...
	public function &getData($id = null)
	{
		if ($this->_item === null)
		{
			...
			if ($table->load($id))
			{
                ...
                $id = $table->id;
                if($id){
					$canEdit = $user->authorise('core.edit', 'com_citybranding.poi.'.$id) || $user->authorise('core.create', 'com_citybranding.poi.'.$id);
				}
				else{
					$canEdit = $user->authorise('core.edit', 'com_citybranding') || $user->authorise('core.create', 'com_citybranding');
				}
                if (!$canEdit && $user->authorise('core.edit.own', 'com_citybranding.poi.'.$id)) {
	...
	{
		// Get the form.
		$form = $this->loadForm('com_citybranding.poi', 'poiform', array('control' => 'jform', 'load_data' => $loadData));
		if (empty($form)) {
			return false;
		}
	...
	{
		$data = JFactory::getApplication()->getUserState('com_citybranding.edit.poi.data', array());
        if (empty($data)) {
            $data = $this->getData();
        }
	...
	{
		$id = (!empty($data['id'])) ? $data['id'] : (int)$this->getState('poi.id');
		...
		if($id) {
            ...
            $authorised = $user->authorise('core.edit', 'com_citybranding.poi.'.$id) || $authorised = $user->authorise('core.edit.own', 'com_citybranding.poi.'.$id);
            if($user->authorise('core.edit.state', 'com_citybranding.poi.'.$id) !== true && $state == 1){ //The user cannot edit the state of the item.
                $data['state'] = 0;
            }
        } else {
        ...
        if(!$id){
        	...
        	$moderation	= $com_citybranding_params->get('newpoineedsmoderation');        
        ...
        if ($table->save($data) === true) {
        	...
        	JFactory::getApplication()->setUserState('com_citybranding.edit.poi.insertid', $table->get('id'));
            return $id;
        } else {
            return false;
        }
    ...
    {
        $id = (!empty($data['id'])) ? $data['id'] : (int)$this->getState('poi.id');
        if(JFactory::getUser()->authorise('core.delete', 'com_citybranding.poi.'.$id) !== true){
            JError::raiseError(403, JText::_('JERROR_ALERTNOAUTHOR'));
            return false;
        }
        $table = $this->getTable();
