 ...
 * @since       1.6
 */
class JFormFieldAreaEdit extends JFormFieldList
{
	/**
	...
	protected function getOptions()
	{
		...
		if ($this->element['parent'] || $jinput->get('option') == 'com_citybranding')
		{
		...
		$query = $db->getQuery(true)
			...
			->join('LEFT', $db->quoteName('#__citybranding_areas') . ' AS b ON a.lft > b.lft AND a.rgt < b.rgt');

		// Filter by the extension type
			/*
		if ($this->element['parent'] == true || $jinput->get('option') == 'com_citybranding')
		{
		...
		*/
		...
		if ($oldCat != 0 && ($this->element['parent'] == true || $jinput->get('option') == 'com_citybranding'))
		{
			// Prevent parenting to children of this item.
			// To rearrange parents and children move the children up, not the parents down.
			$query->join('LEFT', $db->quoteName('#__citybranding_areas') . ' AS p ON p.id = ' . (int) $oldCat)
