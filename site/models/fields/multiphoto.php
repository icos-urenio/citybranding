...
class JFormFieldMultiphoto extends JFormField
{
	...
	/**
	* @var string
	* relative to joomla root (e.g. images/citybranding)
	*/
	protected $imagedir;
...
/**
	...
	protected function getInput()
	{
		...
		$imagedir = (isset($this->element['imagedir']) ? $this->element['imagedir'] : 'images/citybranding');
		...
		JFactory::getDocument()->addStyleSheet(JURI::root(true).'/components/com_citybranding/models/fields/multiphoto/css/blueimp-gallery.min.css');
		JFactory::getDocument()->addStyleSheet(JURI::root(true).'/components/com_citybranding/models/fields/multiphoto/css/citybranding-style.css');
		JFactory::getDocument()->addStyleSheet(JURI::root(true).'/components/com_citybranding/models/fields/multiphoto/css/jquery.fileupload.css');
		JFactory::getDocument()->addStyleSheet(JURI::root(true).'/components/com_citybranding/models/fields/multiphoto/css/jquery.fileupload-ui.css');
		...
		$script[] = '                    <span>'.JText::_('COM_CITYBRANDING_JFIELD_MULTIPHOTO_START').'</span>';
		$script[] = '                </button>';
		$script[] = '            {% } %}';
		$script[] = '            {% if (!i) { %}';
		$script[] = '                <button class="btn btn-warning cancel">';
		$script[] = '                    <i class="icon-remove"></i>';
		$script[] = '                    <span>'.JText::_('COM_CITYBRANDING_JFIELD_MULTIPHOTO_CANCEL').'</span>';
		$script[] = '                </button>';
		$script[] = '            {% } %}';
		$script[] = '        </td>';
		$script[] = '    </tr>';
		...
		JFactory::getDocument()->addScript(JURI::root(true).'/components/com_citybranding/models/fields/multiphoto/js/vendor/jquery.ui.widget.js');		
		JFactory::getDocument()->addScript(JURI::root(true).'/components/com_citybranding/models/fields/multiphoto/js/tmpl.min.js');		
		JFactory::getDocument()->addScript(JURI::root(true).'/components/com_citybranding/models/fields/multiphoto/js/load-image.min.js');		
		JFactory::getDocument()->addScript(JURI::root(true).'/components/com_citybranding/models/fields/multiphoto/js/canvas-to-blob.min.js');		
		JFactory::getDocument()->addScript(JURI::root(true).'/components/com_citybranding/models/fields/multiphoto/js/jquery.blueimp-gallery.min.js');		
		JFactory::getDocument()->addScript(JURI::root(true).'/components/com_citybranding/models/fields/multiphoto/js/jquery.iframe-transport.js');		
		JFactory::getDocument()->addScript(JURI::root(true).'/components/com_citybranding/models/fields/multiphoto/js/jquery.fileupload.js');		
		JFactory::getDocument()->addScript(JURI::root(true).'/components/com_citybranding/models/fields/multiphoto/js/jquery.fileupload-process.js');		
		JFactory::getDocument()->addScript(JURI::root(true).'/components/com_citybranding/models/fields/multiphoto/js/jquery.fileupload-image.js');		
		JFactory::getDocument()->addScript(JURI::root(true).'/components/com_citybranding/models/fields/multiphoto/js/jquery.fileupload-validate.js');		
		JFactory::getDocument()->addScript(JURI::root(true).'/components/com_citybranding/models/fields/multiphoto/js/jquery.fileupload-ui.js');		
		JFactory::getDocument()->addScript(JURI::root(true).'/components/com_citybranding/models/fields/multiphoto/js/multiphoto.js');


		$url = JRoute::_(  JURI::root(true)."/administrator/index.php?option=com_citybranding&task=upload.handler&format=json&id=".$itemId."&imagedir=".$imagedir."&".JSession::getFormToken()."=1" );
		//change controller url if accessed from backend to work with secure JSession token
		if($this->element['side'] == 'frontend'){
			$url = JRoute::_(  JURI::root(true)."/index.php?option=com_citybranding&task=upload.handler&format=json&id=".$itemId."&imagedir=".$imagedir."&".JSession::getFormToken()."=1" );
		}

		//TODO:  get `com_citybranding` as field argument `component`
		$init = array();
		$init[] = "function init() {";
        ...
        $html[] = '            <span>'.JText::_('COM_CITYBRANDING_JFIELD_MULTIPHOTO_ADD_FILES').'</span>';
        $html[] = '            <input type="file" name="files[]" multiple>';
        $html[] = '        </span>';
        $html[] = '        <button type="submit" class="btn btn-primary start">';
        $html[] = '            <i class="icon-upload"></i>';
        $html[] = '            <span>'.JText::_('COM_CITYBRANDING_JFIELD_MULTIPHOTO_START_UPLOAD').'</span>';
        $html[] = '        </button>';
        $html[] = '        <button type="reset" class="btn btn-warning cancel">';
        $html[] = '            <i class="icon-remove"></i>';
        $html[] = '            <span>'.JText::_('COM_CITYBRANDING_JFIELD_MULTIPHOTO_CANCEL_UPLOAD').'</span>';
        $html[] = '        </button>';
        $html[] = '        <button type="button" class="btn btn-danger delete">';
        $html[] = '            <i class="icon-trash"></i>';
        $html[] = '            <span>'.JText::_('COM_CITYBRANDING_JFIELD_MULTIPHOTO_DELETE_FILES').'</span>';
        $html[] = '        </button>';
        ...
        $html[] = '<div class="drop-photos"><span class="dptitle">'.JText::_('COM_CITYBRANDING_JFIELD_MULTIPHOTO_DROP_FILES').'</span>';
